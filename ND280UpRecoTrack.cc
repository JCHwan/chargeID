...
void ND280UpRecoTrack::DoTracking(nd280upconv::TargetType_t dettype){
  ...
  cout<<"running ND280UpRecoTrack::DoTracking "<<fDetType<<endl;
  cout<<"moving forward.. "<<endl;
  cout<<"integrated 2D maps "<<f2d_xy->Integral()<<" "<<f2d_xz->Integral()<<" "<<f2d_yz->Integral()<<endl;
  ...
  cout<<"NHits and Recovar are : "<< fNHitsYZ<<" "<<fRecoCosTheta<<endl; 
...
void ND280UpRecoTrack::CleanMPPCHits(){
  ...
  // Then set it to 0
  cout<<"running ND280UpRecoTrack::CleanMPPCHits"<<endl;
...
void ND280UpRecoTrack::DefineRecoTrack(){
  ...
  SetIsReco(true);
  cout<<"running ND280UpRecoTrack::DefineRecoTrack"<<endl;  
  if(fNumView<2){ 
...
void ND280UpRecoTrack::CalcRecoVar(){
  cout<<"running ND280UpRecoTrack::CalcRecoVar"<<endl; 
...
void ND280UpRecoTrack::CalcNHits(){
  cout<<"runnning ND280UpRecoTrack::CalcNHits"<<endl;
...
void ND280UpRecoTrack::CalcLength(){
  cout<<"running ND280UpRecoTrack::CalcLength"<<endl;
  CalcLengthStraight();
  CalcLengthFit();
...
void ND280UpRecoTrack::CalcOutFV(){
  ...
  bool tempxz=CalcOutFVView(f2d_xz);
  bool tempyz=CalcOutFVView(f2d_yz);
  bool tempxy=CalcOutFVView(f2d_xy);

  if( tempxz && tempyz && !tempxy) fDownOut = true;
  else fDownOut = false;
...
bool ND280UpPID::fCenter(double * sPoint, double * mPoint, double * ePoint, double * center)
{
...
void ND280UpPID::ChargeID(TGraph g){
  double sPoint[2] = {0, 0};
  double mPoint[2] = {0, 0};
  double ePoint[2] = {0, 0};
  double center[2] = {0, 0}; 
  ...
  if (vCenter.X() < 0){
    fChargeID = -1;
    fFit = new TF1("fFit", "TMath::Sqrt([0]*[0] - (x - [1])*(x - [1])) + [2]", center[0] - 3*R, center[0] + 3*R);
  }else{
    fChargeID = +1;
    fFit = new TF1("fFit", "-TMath::Sqrt([0]*[0] - (x - [1])*(x - [1])) + [2]", center[0] - 3*R, center[0] + 3*R);
  }
  ...
  fRadiusID  = fFit->GetParameter(0);
  fCenterYID = fFit->GetParameter(2);
  fCenterZID = fFit->GetParameter(1);
  ...
  //void ND280UpRecoTrack::SetMPPCXY(TH2F *h){f2d_xy=h;} // TODO: Use Clone
//void ND280UpRecoTrack::SetMPPCXZ(TH2F *h){f2d_xz=h;} // TODO: Use Clone
//void ND280UpRecoTrack::SetMPPCYZ(TH2F *h){f2d_yz=h;} // TODO: Use Clone

//void ND280UpRecoTrack::SetMPPCXY_Other(TH2F *h){f2d_xy_other=h;} // TODO: Use Clone
//void ND280UpRecoTrack::SetMPPCXZ_Other(TH2F *h){f2d_xz_other=h;} // TODO: Use Clone
//void ND280UpRecoTrack::SetMPPCYZ_Other(TH2F *h){f2d_yz_other=h;} // TODO: Use Clone
...
double ND280UpRecoTrack::ThreeDPlaneFit(TH2D h1){
        double mean[3] = {};
	Int_t kk=0,kkk=0;
	double tS[3][1000];
	double tot[3]={};
        cout<<"doing three dimensional fit..."<<endl;

        for (Int_t i=0;i<3;i++)
        {
		for(Int_t j=0;j<200;j++){
        	    mean[i] += h1.GetBinContent(i+1,j+1) * j;
		    tot[i]  += h1.GetBinContent(i+1,j+1);
			for(Int_t k=0;k<h1.GetBinContent(i+1,j+1);k++) 
			{tS[i][kk] = j; kk++; kkk++;} 
        	}
	kk=0;
	}
        for (Int_t i=0;i<3;i++)
        {
        mean[i] = mean[i]/(double)tot[i];
	}

	cout<<"mean values are "<<mean[0]<<" "<<mean[1]<<" "<<mean[2]<<endl;
        // Compute the covariance matrix of the points.
        double covar00 = (double)0, covar01 = (double)0, covar02 = (double)0;
        double covar11 = (double)0, covar12 = (double)0;

	double diff0,diff1,diff2;

	cout<<"working hard on three D fit still...total number of E-grid: "<<kkk/3<<endl;
	for(Int_t i=0;i<kkk/3;i++){
         diff0 = tS[0][i] - mean[0];
         diff1 = tS[1][i] - mean[1];
	 diff2 = tS[2][i] - mean[2];	    
        ...
        double det = covar00*covar11 - covar01*covar01;
	cout<<"some fitting results.. covar00, 11 and det: "<<covar00<<" "<<covar01<<" "<<covar02<<" "<<covar11<<" "<<covar12<<" "<<det<<endl;
        if (det != (double)0)
        {
            ...
            mParameters.second[2] = (double)-1;
	    cout<<"Parameter results: "<<mParameters.second[0]<<" "<<mParameters.second[1]<<" "<<mParameters.second[2]<<endl;            
	    double xx= ( (1-mean[2])-mParameters.second[1]*(1-mean[1]) )/mParameters.second[0] + mean[0];
	    double Fangle = 1./ (1. * TMath::Sqrt(xx*xx+1+1)) ;
 	    cout<<"xx is "<<xx<<" Final angle is "<<Fangle<<endl; 	    
	    return Fangle;
        }
	else return -2;
}

TH1D* ND280UpRecoTrack::ThreeDLineFit(TH2D h1){
        double mean[3] = {};
        Int_t kk=0,kkk=0;
        double tS[3][10000];
        double tot[3]={};
        ...
        // Compute the covariance matrix of the points.
        double covar00 = (double)0, covar01 = (double)0, covar02 = (double)0;
        double covar11 = (double)0, covar12 = (double)0, covar22 = (double)0;
	 ...
	 //cout<<"diff test.. "<<tS[0][i]<<" "<<tS[1][i]<<" "<<tS[2][i]<<" "<<diff0<<" "<<diff1<<" "<<diff2<<endl;
        ...
        cout<<"covariance "<<covar00<<" "<<covar01<<" "<<covar02<<" "<<covar11<<" "<<covar12<<" "<<covar22<<endl;

        int NN=3;
	double eign[NN*NN]={covar00, covar01, covar02, covar01, covar11, covar12,
	covar02, covar12, covar22};
	TMatrixDSym matL(NN, eign);
	matL.Print();
	TMatrixDSymEigen meL(matL);
	//meL.Print();
	TVectorD eigenval = meL.GetEigenValues();
	TMatrixD eigenvec = meL.GetEigenVectors(); 

	eigenval.Print();
	eigenvec.Print();

        double parameter1,parameter2,parameter3;
        paraV = new TH1D("","",4,1,4);

        if(eigenval[0]>eigenval[1] && eigenval[0]>eigenval[2]) { cout<<eigenvec(0,0)<<" "<<eigenvec(1,0)<<" "<<eigenvec(2,0)<<endl; parameter1 = eigenvec(0,0); parameter2 = eigenvec(1,0); parameter3 = eigenvec(2,0); }
        else if(eigenval[1]>eigenval[0] && eigenval[1]>eigenval[2]) { cout<<eigenvec(0,1)<<" "<<eigenvec(1,1)<<" "<<eigenvec(2,1)<<endl; parameter1 = eigenvec(0,1); parameter2 = eigenvec(1,1); parameter3 = eigenvec(2,1); }
        else if(eigenval[2]>eigenval[0] && eigenval[2]>eigenval[1]) { cout<<eigenvec(0,2)<<" "<<eigenvec(1,2)<<" "<<eigenvec(2,2)<<endl; parameter1 = eigenvec(0,2); parameter2 = eigenvec(1,2); parameter3 = eigenvec(2,2); }
	paraV ->SetBinContent(1, parameter1);
	paraV ->SetBinContent(2,  parameter2);
	paraV ->SetBinContent(3,  parameter3);
	paraV ->SetBinContent(4,  TMath::Abs(parameter3/TMath::Sqrt(parameter1*parameter1 + parameter2*parameter2 + parameter3*parameter3) ) );
	std::cout<<"internal angle check: "<<paraV->GetBinContent(4)<<std::endl;
        return paraV;	
}
