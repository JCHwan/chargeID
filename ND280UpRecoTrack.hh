...
public:
  ...
  bool IsDownOutFV(){return fDownOut;}
  ...
  // Vertex
  TVector3 fVtxReco;
  double ThreeDPlaneFit(TH2D h1);
  //TVectorD ThreeDLineFit(TH2D h1);
  TH1D* ThreeDLineFit(TH2D h1);
  std::pair<TVector3, TVector3> mParameters;
...
public:
  ...
  int GetChargeID(){return fChargeID;}
  int GetNDF(){return fNDF;}
  double GetRadiusID(){return fRadiusID;}
  double GetCenterYID(){return fCenterYID;}
  double GetCenterZID(){return fCenterZID;}
  double GetChi2(){return fChi2;}
  

  void ChargeID(TGraph g);

  Double_t fit_cir(double * x, double * p)
  {
    double xx = x[0];
    return p[3]*TMath::Sqrt(p[0]*p[0] - (xx-p[1])*(xx-p[1])) - p[2];
  }
